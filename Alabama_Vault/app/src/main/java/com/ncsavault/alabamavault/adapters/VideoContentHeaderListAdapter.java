...
public class VideoContentHeaderListAdapter extends BaseAdapter implements
    ...
    public boolean isHeaderDisable = false;
...
//                    .findViewById(R.id.imgShareButton);
    ...
    public View getHeaderView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.relativeLayout = (FrameLayout) convertView
        ...
        if (isHeaderDisable) {
            ...
            holder.textViewHeader.setTextSize(0);
        } else {
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT);
            holder.relativeLayout.setLayoutParams(layoutParams);
            holder.textViewHeader.setTextSize(20);
        }
...
//        videoImageURL = arrayListVideoDTOs.get(pos).getVideoStillUrl();
        ...
        if (viewType == 1) {
            ...
            if (!isGames) {
                ...
                RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(180, RelativeLayout.LayoutParams.WRAP_CONTENT);
                lp.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
                viewHolder.tvVideoDuration.setLayoutParams(lp);
                viewHolder.tvVideoDuration.setPadding(13,0,0,0);
            }
                ...
                mPostTask = new AsyncTask<Void, Void, Void>() {
                    ...
                    protected Void doInBackground(Void... params) {
                        try {
                            postResult = AppController.getInstance().getServiceManager().getVaultService().
                                    postFavoriteStatus(AppController.getInstance().getModelFacade().getLocalModel().getUserId(),
                                            arrayListVideoDTOs.get(pos).getVideoId(), arrayListVideoDTOs.get(pos).getPlaylistId(),
                                            isFavoriteChecked);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    protected void onPostExecute(Void result) {
                        try {
                            ...
                            {
                                VaultDatabaseHelper.getInstance(context.getApplicationContext()).setFavoriteFlag(1,
                                        arrayListVideoDTOs.get(pos).getVideoId());
...
//                        context.finish();
    ...
    public static boolean containsLocation(Collection<VideoDTO> c, String location) {
        for (VideoDTO o : c) {
            if (o != null && o.getVideoName().equals(location)) {
                return true;
            }
        }
        return false;
        ...
        try {
            ...
            if (charText.length() == 0) {
                ...
                for (VideoDTO dto : listSearch) {
                    System.out.println("search short : "+dto.getVideoShortDescription());
                    System.out.println("search tag : "+dto.getVideoTags());
                    System.out.println("search name : "+dto.getVideoName());
                    if (dto.getVideoShortDescription().toLowerCase(Locale.getDefault()).contains(charText)
